...
#include "ui_windowconnectdatabase.h"

windowConnectDatabase::windowConnectDatabase(QWidget *parent): QDialog(parent), ui(new Ui::windowConnectDatabase)
{
    ui->setupUi(this);
    ...
    QStringList list2 = (QStringList()<<";"<<","<<"Tab (\\t)");
    ...
    ui->time_start->setEnabled(false);
    ui->time_end->setEnabled(false);
    ui->checkAirFlow->setEnabled(false);
    ui->checkCO2->setEnabled(false);
    ui->checkLightState->setEnabled(false);
    ui->checkPres->setEnabled(false);
    ui->checkTemp->setEnabled(false);
    ui->checkTempIr->setEnabled(false);
    ui->checkValve->setEnabled(false);
    ui->checkOutAirDest->setEnabled(false);
    ui->checkOutAirSpeed->setEnabled(false);
    ui->checkOutHumidity->setEnabled(false);
    ui->checkOutLight->setEnabled(false);
    ui->checkOutRain->setEnabled(false);
    ui->checkOutScherming->setEnabled(false);
    ui->checkOutTemp->setEnabled(false);
}

windowConnectDatabase::~windowConnectDatabase()
{
    delete ui;
}

void windowConnectDatabase::on_pushStart_clicked()
{
    ...
    if(dataBase.openConnectie(db_host, db_name, db_user, db_passw))
    {
        ...
        ui->pushAnalyseData->setEnabled(true);
...
void windowConnectDatabase::on_pushRefresh_clicked()
{
    QApplication::setOverrideCursor(Qt::WaitCursor);
    ...
    if(dataBase.openConnectie(db_host, db_name, db_user, db_passw))
    {
        ...
        ui->date_start->setEnabled(true);
        ui->date_end->setEnabled(true);
        ui->time_start->setEnabled(true);
        ui->time_end->setEnabled(true);
        ui->checkAirFlow->setEnabled(true);
        ui->checkCO2->setEnabled(true);
        ui->checkLightState->setEnabled(true);
        ui->checkPres->setEnabled(true);
        ui->checkTemp->setEnabled(true);
        ui->checkTempIr->setEnabled(true);
        ui->checkValve->setEnabled(true);
        ui->checkOutAirDest->setEnabled(true);
        ui->checkOutAirSpeed->setEnabled(true);
        ui->checkOutHumidity->setEnabled(true);
        ui->checkOutLight->setEnabled(true);
        ui->checkOutRain->setEnabled(true);
        ui->checkOutScherming->setEnabled(true);
        ui->checkOutTemp->setEnabled(true);
    }
    else
    {
...
void windowConnectDatabase::new_log_entry(QString newTextLine){
    logText = QTime::currentTime().toString() + "\t" + newTextLine + "\n" + ui->logEntry->toPlainText();
    ui->logEntry->setText(logText);
}

void windowConnectDatabase::buttonStartEnabling()
{
...
void windowConnectDatabase::addExtraSensors(){
...
void windowConnectDatabase::addNormalSensors(){
    for (unsigned int i = 0; i < currentResult.size(); i++){
        std::vector< int > row;
...
void windowConnectDatabase::on_pushExportCSV_clicked()
{
    QApplication::setOverrideCursor(Qt::WaitCursor);

    //exportCSV CSVexporter;
    Exportfiles CSVexporter;
...
void windowConnectDatabase::on_date_start_userDateChanged(const QDate &date)
{
    date_start = date;
}

void windowConnectDatabase::on_date_end_userDateChanged(const QDate &date)
{
    date_end = date;
}

void windowConnectDatabase::on_time_start_timeChanged(const QTime &time)
{
    time_start = time;
}

void windowConnectDatabase::on_time_end_timeChanged(const QTime &time)
{
    time_end = time;
}

void windowConnectDatabase::on_checkPres_clicked(bool checked)
{
    if(checked)
    {
...
void windowConnectDatabase::on_checkCO2_clicked(bool checked)
{
    if(checked)
    {
...
void windowConnectDatabase::on_checkAirFlow_clicked(bool checked)
{
    if(checked)
    {
...
void windowConnectDatabase::on_checkTempIr_clicked(bool checked)
{
    if(checked)
    {
        normalSensors.push_back(5);
    }
    else
    {
        normalSensors.erase(std::remove(normalSensors.begin(), normalSensors.end(), 5), normalSensors.end());
    }
    buttonStartEnabling();
}

void windowConnectDatabase::on_checkLightState_clicked(bool checked)
{
    if(checked)
    {
...
void windowConnectDatabase::on_checkValve_clicked(bool checked)
{
    if(checked)
    {
...
void windowConnectDatabase::on_checkTemp_clicked(bool checked)
{
    if(checked)
    {
...
void windowConnectDatabase::on_checkOutTemp_clicked(bool checked)
{
    if(checked)
    {
...
void windowConnectDatabase::on_checkOutLight_clicked(bool checked)
{
    if(checked)
    {
...
void windowConnectDatabase::on_checkOutAirSpeed_clicked(bool checked)
{
    if(checked)
    {
...
void windowConnectDatabase::on_checkOutAirDest_clicked(bool checked)
{
    if(checked)
    {
...
void windowConnectDatabase::on_checkOutScherming_clicked(bool checked)
{
    if(checked)
    {
...
void windowConnectDatabase::on_checkOutRain_clicked(bool checked)
{
    if(checked)
    {
...
void windowConnectDatabase::on_checkOutHumidity_clicked(bool checked)
{
    if(checked)
    {
...
void windowConnectDatabase::on_pushAnalyseData_clicked()
{
    //this->close();
    this->accept();
}

std::vector< std::vector<QString> > windowConnectDatabase::getDatabaseData()
{
    return currentData;
}

int windowConnectDatabase::getTimeSlot()
{
